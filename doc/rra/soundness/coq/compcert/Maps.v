...
Module PTree <: TREE.
  ...
  (* [CHRC:] This is Xavier's original [set], which shadows when key is already in the map *)
  ...
  (* Added by CHRC *)
  Definition obnd {A B : Type} (a : option A) (f : A -> option B) : option B :=
    match a with
    | Some x => f x
    | None => None
    end.
  (* Added by CHRC *)
  (* [CHRC:] this version of set takes a value [def], and a function [f] that
     takes a value and returns an [option]al value.
     If no key was present, we add [def] and return the resulting tree.
     if key was already present, then
         if [f] <value> gives us a value, we add it at <key> and return the resulting tree.
         if [f] <value> was [None], then [set_] returns [None] *)
  Fixpoint set_ (A: Type) (i: positive) (def: A) (f : A -> option A) (m : t A) {struct i} : option (t A) :=
    match m with
    | Leaf =>
        match i with
        | xH => Some (Node Leaf (Some def) Leaf)
        | xO ii => obnd (set_ ii def f Leaf) (fun x => Some (Node x None Leaf))
        | xI ii => obnd (set_ ii def f Leaf) (fun x => Some (Node Leaf None x))
        end
    | Node l o r =>
        match i with
        | xH => 
              match o with
              | None => Some (Node l (Some def) r)
              | Some v => obnd (f v) (fun x => Some (Node l (Some x) r))
              end
        | xO ii => obnd (set_ ii def f l) (fun x => Some (Node x o r))
        | xI ii => obnd (set_ ii def f r) (fun x => Some (Node l o x))
        end
    end.

  (* [CHRC:] This version of set only adds the element if the key wasn't already present
     "set ONLY if FRESH" *)
  (* Fixpoint set_only_fresh (A : Type) (i : positive) (v : A) (m : t A) {struct i} : t A :=
    match m with
    | Leaf =>
        match i with
        | xH => Node Leaf (Some v) Leaf
        | xO ii => Node (set_only_fresh ii v Leaf) None Leaf
        | xI ii => Node Leaf None (set_only_fresh ii v Leaf)
        end
    | Node l o r as node =>
        match i with
        | xH =>  node
        | xO ii => Node (set_only_fresh ii v l) o r
        | xI ii => Node l o (set_only_fresh ii v r)
        end
    end. *)

    Definition set_only_fresh {A : Type} (i : positive) (v : A) (m : t A) : t A :=
      match get i m with
      | Some _ => m
      | None   => set i v m
      end.

  (* [CHRC:] This version of set only adds the element if the key WAS already present
  "set ONLY if STALE" *)
  Fixpoint set_only_stale (A : Type) (i : positive) (v : A) (m : t A) {struct i} : option (t A) :=
    match m with
    | Leaf => None
    | Node l o r =>
        match i with
        | xH    => Some (Node l (Some v) r)
        | xO ii => obnd (set_only_stale ii v l) (fun x => Some (Node x o r))
        | xI ii => obnd (set_only_stale ii v r) (fun x => Some (Node l o x))
        end
    end.
  ...
  Fixpoint remove (A : Type) (i : positive) (m : t A) {struct i} : t A :=
    ...
    | xH =>
        match m with
        | Leaf => Leaf
        | Node Leaf _o Leaf => Leaf
        | Node l _o r => Node l None r
        end
    | xO ii =>
        match m with
        | Leaf => Leaf
...
Module PMap <: MAP.
  ...
  (* [CHRC:] This version of set only adds the element if the key wasn't already present
     "set ONLY if Fresh" *)
  Definition set_only_f (A : Type) (i : positive) (v : A) (m : t A) :=
    (fst m, PTree.set_only_fresh i v (snd m)).
...
Module I64Indexed <: INDEXED_TYPE.
  Definition t := int64.
  Definition index (i : int64) : positive :=
    match Int64.intval i with
    | Z0 => xH
    | Zpos p => xO p
    | Zneg p => xI p
    end.
  ...
  Proof. unfold index.
    intros [[|x|x] xRan] [[|y|y] yRan]; simpl;
    try discriminate; try reflexivity;
    [ intros _
    | inversion 1; subst y
    | inversion 1; subst y];
    rewrite (proof_irrelevance _ xRan yRan); auto.
  Qed.
  Definition eq := Int64.eq_dec.
End I64Indexed.

Module MemTree := ITree(I64Indexed).